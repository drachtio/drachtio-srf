...
export function getHeaderName(hdr: string): string {
  ...
  return (compactForm as any)[name] || name;
}

function parseResponse(rs: string, m: any) {
...
function parseRequest(rq: string, m: any) {
...
function applyRegex(regex: RegExp, data: any) {
...
function parseMultiHeader(parserFn: any, d: any, h: any) {
  h = h || [];
...
function parseAOR(data: any) {
  const r: any = applyRegex(/((?:[\w\-.!%*_+`'~]+)(?:\s+[\w\-.!%*_+`'~]+)*|"[^"\\]*(?:\\.[^"\\]*)*")?\s*<\s*([^>]*)\s*>|((?:[^\s@"<]@)?[^\s;]+)/g, data);
...
function parseVia(data: any) {
  const r: any = applyRegex(/SIP\s*\/\s*(\d+\.\d+)\s*\/\s*([\S]+)\s+([^\s;:]+)(?:\s*:\s*(\d+))?/g, data);
...
function parseAuthHeader(d: any) {
  const r1: any = applyRegex(/([^\s]*)\s+/g, d);
  const a: any = {scheme: r1[1]};

  let r2: any = applyRegex(/([^\s,"=]*)\s*=\s*([^\s,"]+|"[^"\\]*(?:\\.[^"\\]*)*")\s*/g, d);
  a[r2[1]] = r2[2];
...
function parseAuthenticationInfoHeader(d: any) {
  const a: any = {};
  let r: any = applyRegex(/([^\s,"=]*)\s*=\s*([^\s,"]+|"[^"\\]*(?:\\.[^"\\]*)*")\s*/g, d);
  a[r[1]] = r[2];
...
function parse(data: any, lazy?: boolean) {
  data = data.split(/\r\n(?![ \t])/);
  ...
  if (r) {
    return {
      ...
      params: (r[6].match(/([^;=]+)(=([^;=]+))?/g) || [])
        ...
        .reduce(function(params: any, x) { params[x[0]] = x[1] || null; return params; }, {}),
      headers: ((r[7] || '').split('&').filter((header) => header !== '').reduce((acc: any, header) => {
...
const stringifiers: Record<string, any> = {
  via: function(h: any) {
...
export function stringifySipMessage(m: any) {
  ...
  for (const n in m.headers) {
    if (typeof m.headers[n] === 'string' || !stringifiers[n])
      ...
      s += stringifiers[n](m.headers[n].parsed || m.headers[n], n);
  }
