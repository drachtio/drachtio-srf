  ...
  _onConnect(socket) {
    ...
    obj.pendingRequests.set(msgId, (response) => {
      if (obj.authenticated = ('OK' === response[0])) {
        ...
        try {
          const regex = /^v(\d+)\.(\d+)\.(\d+)/;
          const arr = regex.exec(obj.serverVersion);
          if (arr) {
            const major = parseInt(arr[1]);
            const minor = parseInt(arr[2]);
            if (major < 1 && minor < 9) {
              console.log(`using legacy comms protocol for drachtio server version ${obj.serverVersion}`);
              this.wp.legacy = true;
            }
          }
        } catch (err) {
          console.log(`Error parsing server version: ${obj.serverVersion}, assuming this is v5.0.0 or above`);
        }
      }
      else {
