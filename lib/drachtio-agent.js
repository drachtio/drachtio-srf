...
class DrachtioAgent extends Emitter {
  ...
  _makeRequest(params) {
    ...
    assert.ok(params.socket instanceof net.Socket, 'provided socket is not a net.Socket');
            ...
            if (obj.pendingSipRequests.has(transactionId)) {
              ...
              if (obj.pendingSipAuthRequests.has(cid)) {
                ...
                client.authenticate((err, req) => {
                  ...
                  debug(`auth challenge new transaction ${req.stackTxnId} overwrites ${res.req.stackTxnId}`);
