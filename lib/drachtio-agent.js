...
const node_noop_1 = __importDefault(require("node-noop"));
...
const defer = typeof setImmediate === 'function' ?
    setImmediate : function (fn, ...args) { process.nextTick(fn.bind(fn, ...args)); };
...
class DrachtioAgent extends events_1.EventEmitter {
    ...
    _normalizeParams(socket, uri, options, callback) {
        ...
        callback = callback || node_noop_1.default;
        if (options._socket) {
    ...
    _makeRequest(params) {
        ...
        obj.pendingRequests.set(msgId, (token, msgStr) => {
            if (token[0] === 'OK') {
                ...
                const req = new request_1.default(new message_1.default(msgStr), meta);
                req.agent = this;
    ...
    uac(socket, request_uri, options, callback) {
    ...
    sendResponse(res, opts, callback, fnAck) {
        ...
        if ((callback && typeof callback === 'function') || fnAck) {
            obj.pendingRequests.set(msgId, (token, msgStr, meta) => {
                ...
                const responseMsg = new message_1.default(msgStr);
                res.meta = meta;
                if (callback) {
    ...
    sendAck(method, dialogId, req, res, opts, callback) {
        ...
        if (callback) {
            obj.pendingRequests.set(msgId, (token, msgStr) => {
                ...
                callback(null, new message_1.default(msgStr));
            });
        }
    }
                    ...
                    if ('network' === source && sipMsg.type === 'request') {
                        ...
                        if (obj.pendingSipRequests.has(transactionId)) {
                            ...
                            if (obj.pendingSipAuthRequests.has(cid)) {
                                ...
                                client.authenticate((err, authReq) => {
                                    if (!authReq) {
                                        sr.req.emit('response', res, ack);
                                        return;
                                    }
                                    res.req.listeners('response').forEach((l) => { authReq.on('response', l); });
