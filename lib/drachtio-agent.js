...
class DrachtioAgent extends Emitter {
  ...
  _makeRequest(params) {
    ...
    if (!typeSocket(params.socket)) {
      params.callback(new Error('provided socket is not a net.Socket or tls.TLSSocket'));
      return;
    }
    if (params.socket.destroyed) {
      params.callback(new Error('provided socket has been destroyed'));
      return;
    }
      ...
      case 'sip':
        ...
        if (token.length > 9) {
          ...
          if ('network' === source && sipMsg.type === 'request') {
            ...
            else if ('UPDATE' === req.method && obj.pendingSipRequests.has(transactionId)) {
