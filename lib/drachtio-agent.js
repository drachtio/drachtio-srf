...
class DrachtioAgent extends Emitter {
  ...
  _prepareSipRequestMessage(params) {
  ...
  _makeRequest(params) {
    ...
    const { m, msg } = this._prepareSipRequestMessage(params);
    ...
    obj.pendingRequests.set(msgId, (token, responseMessage) => {
      if (token[0] === 'OK') {
        ...
        // Note: The SipMessage for the request ('m') was created in _prepareSipRequestMessage.
        // However, the callback provides 'responseMessage', which is the actual message from the server.
        // For a new request, we need to create a Request object based on the *original* SipMessage 'm',
        // but associate it with the metadata from the server's response acknowledgment.
        // The actual content of the server's response (if it's a SIP response) will be handled
        // by _handleSipResponse. This callback here is for the ack of sending the request.
        const req = new Request(m, meta) ; // Using 'm' (original SipMessage) for Request object
        req.agent = this ;
        req.socket = obj.socket ; // Use the socket from the initial params
        ...
        if (params.options.method !== 'CANCEL') {
          obj.pendingSipRequests.set(transactionId,  {
            req: req // req here is the new Request object based on original 'm'
          }) ;
        }
  ...
  _setupPendingResponse(res, msgId, obj, callback, fnAck) {
    if ((callback && typeof callback === 'function') || fnAck) {
      obj.pendingRequests.set(msgId, (token, msg, meta) => {
        obj.pendingRequests.delete(msgId) ;
        if ('OK' !== token[0]) {
          // Ensure callback is called even on error, if provided
          return callback && callback(token[1]) ;
        }
  ...
  sendResponse(res, opts, callback, fnAck) {
    ...
    this._setupPendingResponse(res, msgId, obj, callback, fnAck);
  ...
  _handleSipRequest(socket, sipMsg, meta, obj, token) {
    let res, sr;
  ...
  _handleSipResponse(socket, sipMsg, meta, obj, token) {
    let res, sr;
  ...
  _handleUnsolicitedSipRequest(socket, sipMsg, meta, obj, token) {
    let res;
  ...
  _handleSipMsg(socket, msg, token, rawMsg, obj) {
    let sipMsg;
    if (!obj) {
    ...
    if (token.length > 9) {
      if ('network' === source && sipMsg.type === 'request') {
        this._handleSipRequest(socket, sipMsg, meta, obj, token);
      }
      else if ('network' === source && sipMsg.type === 'response') {
        this._handleSipResponse(socket, sipMsg, meta, obj, token);
      }
      else if ('application' === source && sipMsg.type === 'request' && transactionId === 'unsolicited') {
        this._handleUnsolicitedSipRequest(socket, sipMsg, meta, obj, token);
      }
    }
  }

  _handleResponseMsg(socket, msg, token, rawMsg, obj) {
    if (!obj) {
      debug('socket not found, message discarding');
      return ;
    }
  ...
  _handleCdrMsg(socket, msg, token, rawMsg, obj) {
  ...
  _onMsg(socket, msg) {
    ...
    let rawMsg ; // rawMsg is derived within each handler

    switch (token[1]) {
      case 'sip':
        this._handleSipMsg(socket, msg, token, rawMsg, obj);
        break ;

      case 'response':
        this._handleResponseMsg(socket, msg, token, rawMsg, obj);
        break ;

      case 'cdr:attempt':
      case 'cdr:start':
      case 'cdr:stop':
        this._handleCdrMsg(socket, msg, token, rawMsg, obj);
        break ;
