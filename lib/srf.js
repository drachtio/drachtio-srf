...
class Srf extends Emitter {
  ...
  constructor(app) {
    ...
    assert(typeof app === 'function' || typeof app === 'object',
      'argument \'app\' must be provided and must be either a drachtio app or connect opts') ;
  ...
  createBackToBackDialogs(req, res, uri, opts, cb) {
    ...
    async.detectSeries(
      ...
      (successUri) => {
        ...
        // success
        proxyResponseHeaders.forEach((hdr) => {
          if (finalUacSuccess.res.has(hdr)) { opts.headers[hdr] = finalUacSuccess.res.get(hdr) ; } }) ;
  ...
  */
  proxyRequest(req, destination, opts, callback) {
    assert(typeof destination === 'string' || _.isArray(destination),
