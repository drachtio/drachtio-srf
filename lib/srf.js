...
Srf.prototype.createBackToBackDialogs = function( req, res, uri, opts, cb ) {
  ...
  if( !opts.headers.from) { opts.callingNumber = req.callingNumber; }
  if( !opts.headers.to ) { opts.calledNumber = req.calledNumber; }

  opts.localSdp = opts.localSdp || req.body ;
  ...
  async.detectSeries( 
    ...
    function truthTest( uri, callback) {
      ...
      function( provisionalRes ) {
        if( provisionalRes.status > 100 ) {
          ...
          var opts = {} ;
          if( provisionalRes.body ) { opts.body = provisionalRes.body ; }
    ...
    function( successUri) {
      ...
      // success 
      opts = {} ;
      opts.localSdp = finalUacSuccess.remote.sdp ;
