...
class Srf extends Emitter {
  ...
  */
  createB2BUA(req, res, uri, opts, cbRequest, cbProvisional, callback) {
    ...
    // new signature: uri, opts, {cbRequest, cbProvisional}, callback
    if (cbRequest && typeof cbRequest === 'object') {
      debug(`createB2BUA: using new signature with object callbacks, keys: ${Object.keys(cbRequest).join(', ')}`);
      callback = cbProvisional;
      const obj = cbRequest;
      cbRequest = obj.cbRequest || noop;
      cbProvisional = obj.cbProvisional || noop;
      cbFinalizedUac = obj.cbFinalizedUac || noop;
      debug(`createB2BUA: extracted cbRequest=${typeof cbRequest}, cbProvisional=${typeof cbProvisional}, cbFinalizedUac=${typeof cbFinalizedUac}`);
    }
    else {
      debug(`createB2BUA: using old signature, cbRequest=${typeof cbRequest}, cbProvisional=${typeof cbProvisional}`);
      cbProvisional = cbProvisional || noop;
      cbRequest = cbRequest || noop;
    }
    ...
    // Track early dialog state for UPDATE forwarding
    let earlyDialogToHeader = null;
    let earlyDialogContactUri = null;
    let bLegCSeqNumber = null; // Track B-leg CSeq number, initialized from actual INVITE CSeq
    ...
    function handleUACSent(err, uacReq) {
      if (err) {
        ...
        // Capture the actual CSeq number from the B-leg INVITE
        const inviteCSeqHeader = uacReq.get ? uacReq.get('CSeq') : null;
        if (inviteCSeqHeader) {
          const cseqMatch = inviteCSeqHeader.match(/^(\d+)/);
          if (cseqMatch) {
            bLegCSeqNumber = parseInt(cseqMatch[1], 10);
            debug(`createB2BUA: Captured B-leg INVITE CSeq: ${bLegCSeqNumber}`);
          }
        }
        if (bLegCSeqNumber === null) {
          bLegCSeqNumber = 1; // Fallback to 1 if we couldn't get the CSeq
          debug('createB2BUA: Could not get B-leg INVITE CSeq, defaulting to 1');
        }
        ...
        // Handle UPDATE requests from A party
        if (opts.forwardUpdate) {
          // Forward UPDATE to B-leg during early dialog
          debug('createB2BUA: forwardUpdate option set - setting up early dialog UPDATE forwarding');
          req.on('update', async (_updateReq, _res) => {
            debug('createB2BUA: received UPDATE from A party, forwarding to B-leg');
            try {
              const updateResponse = await forwardEarlyDialogUpdate(_updateReq, uacReq, earlyDialogToHeader, earlyDialogContactUri, opts.proxy);
              debug(`createB2BUA: UPDATE forwarded successfully, got ${updateResponse.status} response`);
              _res.send(updateResponse.status, updateResponse.reason || 'OK', {
                body: updateResponse.body || undefined
              });
            } catch (updateErr) {
              debug(`createB2BUA: Error forwarding UPDATE to B-leg: ${updateErr.message}`);
              _res.send(500, 'Server Internal Error');
            }
          });
        } else {
          // Default behavior: respond with current SDP
          req.on('update', (_updateReq, _res) => {
    ...
    /**
     * Forward UPDATE request from A-leg to B-leg during early dialog.
     * This is used for SIP preconditions (RFC 3312) where UPDATE is sent before 200 OK.
     */
    function forwardEarlyDialogUpdate(updateReq, uacReq, toHeader, contactUri, proxy) {
      return new Promise((resolve, reject) => {
        if (!uacReq || !uacReq.agent) {
          return reject(new Error('No B-leg agent available for UPDATE forwarding'));
        }

        const agent = uacReq.agent;

        // Use Contact URI from 183 response if available, otherwise fall back to original INVITE URI
        // Per SIP RFC, mid-dialog requests should use the Contact URI from the remote party
        const targetUri = contactUri || uacReq.uri;
        debug(`createB2BUA: forwardEarlyDialogUpdate - using targetUri=${targetUri} (contactUri=${contactUri}, uacReq.uri=${uacReq.uri})`);

        // Build UPDATE request options - send as new request with proper SIP headers
        const updateOpts = {
          method: 'UPDATE',
          uri: targetUri,
          headers: {},
          _socket: uacReq.socket
        };

        // Add proxy if specified
        if (proxy) {
          updateOpts.proxy = proxy.startsWith('sip:') ? proxy : `sip:${proxy}`;
        }

        // Copy essential headers from B-leg INVITE for early dialog correlation
        const bLegCallId = uacReq.get ? uacReq.get('Call-ID') : null;
        const bLegFrom = uacReq.get ? uacReq.get('From') : null;

        if (bLegCallId) {
          updateOpts.headers['Call-ID'] = bLegCallId;
        }
        if (bLegFrom) {
          updateOpts.headers['From'] = bLegFrom;
        }
        if (toHeader) {
          updateOpts.headers['To'] = toHeader;
        }

        // Add Contact header for the B-leg UPDATE
        // Use the Contact from the original INVITE if available, otherwise generate one
        const uacContact = uacReq.get ? uacReq.get('Contact') : null;
        if (uacContact) {
          updateOpts.headers['Contact'] = uacContact;
          debug(`createB2BUA: forwardEarlyDialogUpdate - using Contact from INVITE: ${uacContact}`);
        }

        // Set proper CSeq for the B-leg dialog
        // Increment the B-leg CSeq counter and use it for the UPDATE
        bLegCSeqNumber++;
        updateOpts.headers['CSeq'] = `${bLegCSeqNumber} UPDATE`;
        debug(`createB2BUA: forwardEarlyDialogUpdate - using CSeq: ${bLegCSeqNumber} UPDATE`);

        // Headers that should NOT be forwarded (hop-by-hop or dialog-specific)
        const IMMUTABLE_HEADERS = ['via', 'from', 'to', 'call-id', 'cseq', 'max-forwards', 'content-length', 'route', 'record-route', 'contact'];

        // Copy all headers from incoming UPDATE except immutable ones
        if (updateReq.headers) {
          Object.keys(updateReq.headers).forEach(hdr => {
            if (!IMMUTABLE_HEADERS.includes(hdr.toLowerCase())) {
              updateOpts.headers[hdr] = updateReq.headers[hdr];
            }
          });
        }

        // Include SDP body if present
        if (updateReq.body) {
          updateOpts.body = updateReq.body;
        }

        debug(`createB2BUA: Forwarding UPDATE to B-leg - uri=${updateOpts.uri}, callId=${bLegCallId}`);

        agent.request(updateOpts, (err, sentReq) => {
          if (err) {
            debug(`createB2BUA: Error sending UPDATE to B-leg: ${err.message}`);
            return reject(err);
          }

          sentReq.on('response', (response) => {
            debug(`createB2BUA: Received UPDATE response from B-leg: ${response.status}`);
            resolve(response);
          });

          sentReq.on('error', (reqErr) => {
            debug(`createB2BUA: Error on UPDATE request to B-leg: ${reqErr.message}`);
            reject(reqErr);
          });
        });
      });
    ...
    const handleUACProvisionalResponse = async(provisionalRes, uacReq) => {
      debug(`handleUACProvisionalResponse: called with status ${provisionalRes.status}, cbProvisional=${typeof cbProvisional}`);

      // Capture To header and Contact URI from 18x responses for early dialog UPDATE forwarding
      if (provisionalRes.status >= 180 && provisionalRes.status < 200) {
        const toHeader = provisionalRes.get ? provisionalRes.get('To') : null;
        if (toHeader && !earlyDialogToHeader) {
          earlyDialogToHeader = toHeader;
          debug(`createB2BUA: Captured early dialog To header from ${provisionalRes.status}: ${earlyDialogToHeader}`);
        }
        // Capture Contact URI for UPDATE Request-URI (per SIP RFC, mid-dialog requests use Contact)
        const contactHeader = provisionalRes.get ? provisionalRes.get('Contact') : null;
        if (contactHeader && !earlyDialogContactUri) {
          // Extract URI from Contact header (handle <uri> format)
          const match = contactHeader.match(/<([^>]+)>/);
          earlyDialogContactUri = match ? match[1] : contactHeader;
          debug(`createB2BUA: Captured early dialog Contact URI from ${provisionalRes.status}: ${earlyDialogContactUri}`);
        }
        // If B-leg response has RSeq (100rel), drachtio will send PRACK which increments CSeq
        const rseq = provisionalRes.get ? provisionalRes.get('RSeq') : null;
        if (rseq && bLegCSeqNumber !== null) {
          bLegCSeqNumber++;
          debug(`createB2BUA: B-leg 183 has RSeq (100rel), PRACK will be sent. Incrementing CSeq to ${bLegCSeqNumber}`);
        }
      }
        ...
        if (propagateProvisional) {
          ...
          if (provisionalRes.body) {
            try {
              ...
              opts.body = sdpA;
              debug(`handleUACProvisionalResponse: calling cbProvisional before res.send`);
            ...
            debug(`handleUACProvisionalResponse: calling cbProvisional before res.send (no body)`);
            cbProvisional(provisionalRes);
