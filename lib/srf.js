...
class Srf extends Emitter {
   ...
   * @param  {Srf~uasOptions} opt configuration options
   * @return {Promise} Promise that is resolves with Dialog on success, rejects with error otherwise
   */
  createUAS(req, res, opts = {}) {
    debug('createUAS starting..');

    opts.headers = opts.headers || {} ;

    const generateSdp = typeof opts.localSdp === 'function' ?
      opts.localSdp : () => { return Promise.resolve(opts.localSdp); };


    return new Promise((resolve, reject) => {

      debug('createUAS generating SDP');
      generateSdp()
        .then((localSdp) => {
          debug(`generated SDP ${localSdp}`);
          ...
          req.on('cancel', () => {
            ...
            reject(new SipError(487, 'Request Terminated')) ;
          }) ;
        });
    });
  }
  ...
  * @param {Srf~provisionalResponseCallback} [cbProvisional]  callback that passes on provisional responses
   * @return {Promise}     Promise that is resolves with Dialog on success, rejects with error otherwise
  */
  createUAC(uri, opts, cbRequest, cbProvisional) {
  ...
  */
  createB2BUA(req, res, uri, opts, cbRequest, cbProvisional) {
    assert.ok(typeof uri === 'string');   // minimally, we must have a request-uri
    ...
    debug(`createB2BUA: target uri is ${uri}, opts: ${JSON.stringify(opts)}`);

    opts.method = req.method ;

    var proxyRequestHeaders = opts.proxyRequestHeaders || [] ;
    var proxyResponseHeaders = opts.proxyResponseHeaders || [] ;
    debug(`createB2BUA: proxyResponseHeaders ${proxyResponseHeaders}`);
    ...
    return new Promise((resolve, reject) => {
      ...
      debug('createB2BUA: creating UAC..');

      this.createUAC(uri, opts, handleUACSent, handleUACProvisionalResponse)
        .then((uac) => {

          //success establishing uac (B) leg, now establish uas (A) leg
          debug('createB2BUA: successfully created UAC..');

          this.createUAS(req, res, {
            ...
            .then((uas) => {
              ...
              resolve({uac, uas});  // successfully connected!  resolve promise with both dialogs
            })
            .catch((err) => {
        ...
        .catch((err) => {
          debug(`createB2BUA: received non-success ${err.status} on uac leg`);
          res.send(err.status) ;    // failed B: propogate failure to A
          reject(err);
        });
    });
  }
  ...
  * (note: this method is deprecated, please try to migrate to createUAS)
  * @param  {Request}   req    incoming drachtio Request object, received in app.invite(...) method
  * @param  {Response}  res    drachtio Response passed with incoming request
  ...
  * (note: this method is deprecated, please try to migrate to createUAC)
  ...
  */
  proxyRequest(req, destination, opts, callback) {
    assert(typeof destination === 'string' || Array.isArray(destination),
      '\'destination\' is required and must be a string or an array of strings') ;
