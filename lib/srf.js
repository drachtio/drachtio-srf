...
function copyAllHeaders(headers, obj, copyContactHeaders = false) {
  if (headers) {
    const filteredNoncopyableHdrs = copyContactHeaders
      ? noncopyableHdrs.filter((h) => h !== 'contact')
      : noncopyableHdrs;

    Object.keys(headers).forEach((h) => {
      if (!filteredNoncopyableHdrs.includes(h) && !obj[h]) {
        obj[h] = headers[h];
      }
    });
  }
  ...
  createB2BUA(req, res, uri, opts, cbRequest, cbProvisional, callback) {
    ...
    function copyUACHeadersToUAS(uacRes) {
      ...
      if (proxyResponseHeaders[0] === 'all') {
        ...
        // For redirect responses, due to RFC3261 session 23.3.1 to 23.3.3,
        // B2BUA must copy contact headers from the response to the A party
        copyAllHeaders(resHeaders, headers, uacRes.status >= 300 && uacRes.status < 400);
      }
      else {
