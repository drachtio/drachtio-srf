  ...
  */
  createUAC(uri, opts, cbRequest, cbProvisional, callback) {
    ...
    const __x = (callback) => {
      ...
      let sdpFromLatestUpdateRequest = null;
      const launchRequest = (uri, method, opts, callback) => {
        ...
        (err, req) => {
          ...
          req.on('update', (_updateReq, res) => {
            ...
            sdpFromLatestUpdateRequest = res.body;
            res.send(200, {body: opts.localSdp}) ;
            debug({sdpFromLatestUpdateRequest}, 'createUAC: got UPDATE during INVITE, responding with initial offer');
          }) ;
          ...
          req.on('response', (res, ack) => {
            if (res.status < 200) {
              ...
              if ((200 === res.status && method === 'INVITE') ||
                ...
                if (sdpFromLatestUpdateRequest && !res.body) {
                  debug({sdpFromLatestUpdateRequest}, 'createUAC: no SDP in response, using SDP from latest UPDATE');
                  res.body = sdpFromLatestUpdateRequest;
                }
                const dialog = new Dialog(this, 'uac', {req: req, res: res, auth: opts.auth}) ;
                if (req._dialogState) {
                  dialog.stateEmitter = {
