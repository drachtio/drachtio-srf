...
class Srf extends Emitter {
  ...
  */
  createB2BUA(req, res, uri, opts, cbRequest, cbProvisional, callback) {
      ...
      if (err) {
        ...
        if (opts.forwardUpdate) {
          ...
          req.on('update', async (_updateReq, _res) => {
            ...
            try {
              const updateResponse = await forwardEarlyDialogUpdate(_updateReq, uacReq, earlyDialogToHeader, opts.proxy);
     ...
     */
    function forwardEarlyDialogUpdate(updateReq, uacReq, toHeader, proxy) {
      return new Promise((resolve, reject) => {
    ...
    const handleUACProvisionalResponse = async(provisionalRes, uacReq) => {
      ...
      // Capture To header from 18x responses for early dialog UPDATE forwarding
