...
class Srf extends Emitter {
  ...
  createUAS(req, res, opts = {}, callback) {
      ...
      generateSdp()
        .then((content) => {
          res.send(req.method === 'INVITE'  ? 200 : 202, {
            ...
            const dialog = new Dialog(self, 'uas', {req: req, res: res, sent: response}) ;
            ...
            if (req.method === 'INVITE') {
              ...
              dialog.once('ack', () => { if (!req.canceled) callback(null, dialog) ;}) ;
            }
            else {
    ...
    if (callback) return _createUAS(callback) ;

    return new Promise((resolve, reject) => {
      _createUAS((err, dialog) => {
  ...
  */
  createUAC(uri, opts, cbRequest, cbProvisional, callback) {
    ...
    if (callback) return _createUAC(callback) ;

    return new Promise((resolve, reject) => {
      _createUAC((err, dialog) => {
        if (err) return reject(err);
        resolve(dialog);
      });
    });
  ...
  */
  createB2BUA(req, res, uri, opts, cbRequest, cbProvisional, callback) {
    ...
    function _createB2BUA(callback) {
      debug(`createB2BUA: creating UAC, opts: ${JSON.stringify(opts)}`);

      opts._socket = req.socket ;

      self.createUAC(opts, {cbRequest: handleUACSent, cbProvisional: handleUACProvisionalResponse})
        .then((uac) => {
    ...
    if (callback) return _createB2BUA(callback) ;

    return new Promise((resolve, reject) => {
      _createB2BUA((err, dialogs) => {
        if (err) return reject(err);
        resolve(dialogs);
      });
    });
