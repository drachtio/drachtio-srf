...
const Socket = net_1.default.Socket;
...
class Srf extends events_1.EventEmitter {
    ...
    createUAS(req, res, opts = {}, callback) {
        ...
        const __send = (content, cb) => {
            ...
            return res.send(req.method === 'INVITE' ? 200 : 202, {
                ...
                let dialog;
                try {
                    dialog = new dialog_1.default(this, 'uas', { req: req, res: res, sent: response });
                }
                catch (err) {
    ...
    createUAC(uri, opts, cbRequest, cbProvisional, callback) {
        ...
        const __x = (cb) => {
            ...
            assert_1.default.ok(!!opts.uri, 'uri must be specified');
            ...
            const launchRequest = (launchUri, launchMethod, launchOpts, lCb) => {
                ...
                this._app.request({
                    ...
                    cbRequest(null, req);
                    req.on('update', (_updateReq, response) => {
                    ...
                    req.on('response', (res, ack) => {
                        if (res.status < 200) {
                            ...
                            if (is3pcc && 200 === res.status && !!res.body) {
                                    ...
                                    return lCb(null, {
                                        ...
                                        ack: (localSdp) => {
                                            return new Promise((resolve, reject) => {
                                                ...
                                                let dialog;
                                                try {
                                                    dialog = new dialog_1.default(this, 'uac', { req: req, res: res, auth: launchOpts.auth });
                                                }
                                                catch (err) {
                                ...
                                const p = sdp_transform_1.default.parse(res.body);
                                p.direction = 'recvonly';
                                const bhSdp = sdp_transform_1.default.write(p);
                                ack({
                                    body: bhSdp
                                });
                            }
    ...
    createB2BUA(req, res, uri, opts, cbRequest, cbProvisional, callback) {
        ...
        assert_1.default.ok(typeof opts.uri === 'string');
        ...
        if (!(opts.headers.from || opts.headers.From) && !opts.callingNumber) {
            opts.callingNumber = req.callingNumber;
        }
        if (!(opts.headers.from || opts.headers.From) && !opts.callingName) {
            opts.callingName = req.callingName;
        }
        if (!(opts.headers.to || opts.headers.To) && !opts.calledNumber) {
            opts.calledNumber = req.calledNumber;
        }
        ...
        const handleUACProvisionalResponse = async (provisionalRes) => {
            if (provisionalRes.status > 101) {
                ...
                if (propagateProvisional) {
                    const resOpts = { headers: copyUACHeadersToUAS(provisionalRes) };
                    let fnAck = null;
    ...
    addDialog(dialog) {
        ...
        log('Srf#addDialog: adding dialog with id %s type %s, dialog count is now %d ', dialog.id, dialog.dialogType, this._dialogs.size);
    }
    removeDialog(dialog) {
...
Srf.Dialog = dialog_1.default;
Srf.SipError = sip_error_1.default;
Srf.parseUri = parser.parseUri;
Srf.stringifyUri = parser.stringifyUri;
Srf.SipMessage = message_1.default;
Srf.SipRequest = request_1.default;
Srf.SipResponse = response_1.default;
Srf.DialogState = _DialogState;
Srf.DialogDirection = _DialogDirection;
