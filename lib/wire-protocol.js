...
module.exports = class WireProtocol extends Emitter {
  ...
  _onData(socket, msg) {
    ...
    while ( index != -1 ) {
      ...
      // We want to check if this is a 64 bit integer that is up to 20 digits 
      if ( !messageSizeStr.match(/^\d{1,20}/) )
        return

      // Second check, if the parseInt fails then this is a big error
      ...
      if ( currentMsg.length >= totalLength ) {
        ...
        this.emit('msg', socket, messageData, { messageSize, start, fullMsg: currentMsg });
      }

      obj.incomingMsg = [...obj.incomingMsg].slice(totalLength, currentMsg.length).join('')
