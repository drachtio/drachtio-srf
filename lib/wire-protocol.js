...
module.exports = class WireProtocol extends Emitter {
  ...
  connect(opts) {
    ...
    this.reconnectVars = {};
    // Seed the retry state before the first connect attempt can fail.
    // initializeRetryVars() is otherwise only called from the socket 'connect'
    // handler, so a client whose very first attempt is refused reaches
    // _onConnectionGone() with retryDelay/retryBackoff/attempts all undefined:
    // Math.floor(undefined * 1.7) is NaN, setTimeout(fn, NaN) fires on the next
    // tick, and maxAttempts cannot cap it either because undefined + 1 is NaN.
    // The result is an unthrottled reconnect loop for as long as the server is
    // not listening. The retry path re-enters net.connect() directly rather
    // than connect(), so seeding here runs once per connect() and does not
    // disturb the backoff accumulated across retries.
    if (this.isClient) this.initializeRetryVars();
